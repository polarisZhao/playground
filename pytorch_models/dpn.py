...
class DPN(nn.Module):
    ...
    def __init__(self, num_init_features=64, k_R=96, G=32,
        ...
        bw = 256
        R = int((k_R*bw)/256)
        print("R:{0:}, bw:{1:}, inc: {2:}, G:{3:}".format(R, bw, inc_sec[0], G))
        blocks['conv2_1'] = DualPathBlock(num_init_features, R, R, bw, inc_sec[0], G, 'proj')
        in_chs = bw + 3 * inc_sec[0]
        for i in range(2, k_sec[0]+1):
            blocks['conv2_{}'.format(i)] = DualPathBlock(in_chs, R, R, bw, inc_sec[0], G, 'normal')
            in_chs += inc_sec[0]

        # conv3
        bw = 512
        R = int((k_R*bw)/256)
        blocks['conv3_1'] = DualPathBlock(in_chs, R, R, bw, inc_sec[1], G, 'down')
        in_chs = bw + 3 * inc_sec[1]
        for i in range(2, k_sec[1]+1):
            blocks['conv3_{}'.format(i)] = DualPathBlock(in_chs, R, R, bw, inc_sec[1], G, 'normal')
            in_chs += inc_sec[1]

        # conv4
        bw = 1024
        R = int((k_R*bw)/256)
        blocks['conv4_1'] = DualPathBlock(in_chs, R, R, bw, inc_sec[2], G, 'down')
        in_chs = bw + 3 * inc_sec[2]
        for i in range(2, k_sec[2]+1):
            blocks['conv4_{}'.format(i)] = DualPathBlock(in_chs, R, R, bw, inc_sec[2], G, 'normal')
            in_chs += inc_sec[2]

        # conv5
        bw = 2048
        R = int((k_R*bw)/256)
        blocks['conv5_1'] = DualPathBlock(in_chs, R, R, bw, inc_sec[3], G, 'down')
        in_chs = bw + 3 * inc_sec[3]
        for i in range(2, k_sec[3]+1):
            blocks['conv5_{}'.format(i)] = DualPathBlock(in_chs, R, R, bw, inc_sec[3], G, 'normal')
            in_chs += inc_sec[3]
...
def dpn92(num_classes=1000):
    return DPN(num_init_features=64, k_R=96, G=32, k_sec=(3,4,20,3), 
    	       inc_sec=(16,32,24,128), num_classes=num_classes)

def dpn98(num_classes=1000):
    return DPN(num_init_features=96, k_R=160, G=40, k_sec=(3,6,20,3), 
    	       inc_sec=(16,32,32,128), num_classes=num_classes)

def dpn131(num_classes=1000):
    return DPN(num_init_features=128, k_R=160, G=40, k_sec=(4,8,28,3),
               inc_sec=(16,32,32,128), num_classes=num_classes)

def dpn107(num_classes=1000):
    return DPN(num_init_features=128, k_R=200, G=50, k_sec=(4,8,20,3), 
    	       inc_sec=(20,64,64,128), num_classes=num_classes)

dpn_clk = dpn107()
inputs = torch.rand(2, 3, 224, 224)
print(dpn_clk(inputs).size())
